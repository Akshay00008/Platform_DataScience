        ...
        try:
            ...
            if os.path.exists(faiss_path_1):
                try:
                    ...
                    logger.info(f"Loaded FAISS index from {faiss_path_1}")
                except Exception as e:
                    logger.error(f"Failed to load FAISS index from {faiss_path_1}: {e}")
            else:
                logger.info(f"FAISS index does not exist at {faiss_path_1}")
            ...
            if os.path.exists(faiss_path_2):
                try:
                    ...
                    logger.info(f"Loaded FAISS index from {faiss_path_2}")
                except Exception as e:
                    logger.error(f"Failed to load FAISS index from {faiss_path_2}: {e}")
            else:
                logger.info(f"FAISS index does not exist at {faiss_path_2}")

            if not new_vector_store and not new_vector_store_1:
                logger.error("No FAISS indexes were loaded; returning empty context.")
            ...
            if new_vector_store:
                ...
                for doc in retrieved_docs:
                    if getattr(doc, 'id', None) not in seen_docs:
                        combined_docs.append(doc)
                        seen_docs.add(getattr(doc, 'id', None))
            ...
            logger.info("Combined documents retrieved successfully.")
