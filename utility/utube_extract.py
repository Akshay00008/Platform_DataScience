...
CHATBOT_ID = "68418a5ea750b0a21067158a"
VERSION_ID = "68418a5ea750b0a21067158e"
...
def insert_video_to_db(video_data):
    ...
    try:
        ...
        print(f"MongoDB insert error for video '{video_data.get('url')}': {e}")
        return None


def get_video_details_from_playlist(playlist_url, inserted_count=0):
    """Fetch video details from YouTube playlist and insert into MongoDB."""
    youtube = build('youtube', 'v3', developerKey=API_KEY)
    ...
    while True:
        ...
        for item in pl_response.get('items', []):
            ...
            title = video_snippet.get('title', 'No title available')
            description = video_snippet.get('description', 'No description available')
            keywords = video_snippet.get('tags', [])  # List or empty list if not present
            ...
            inserted_id = insert_video_to_db(video_data)
            if inserted_id:
                inserted_count += 1
